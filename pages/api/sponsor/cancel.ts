...
import {FirestoreDataConverter, WithFieldValue,
  DocumentData,
  QueryDocumentSnapshot, Timestamp, FieldValue} from "firebase-admin/firestore";
import {z} from "zod";
...
const Cancel = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const memberDoc = firestore.doc(`members/${member}`).withConverter(Utils.memberConverter);

  const batch = firestore.batch();
  await stripe.subscriptionItems.del(stripeSubscriptionItem).catch(async (err) => {
    if (!err.message.includes("A subscription must have at least one active plan. To cancel a subscription, please use the cancel API endpoint on /v1/subscriptions.")) {
      throw new Error(err);
    }
  ...
  batch.delete(makerSponsorshipDoc);
