...
export const prettifyJSON = (json: Record<string, any>) =>
  JSON.stringify(json, null, 2).replace("{", "").replace("}", "");
...
const quickReplyInfoToQuickReply = (
  info: QuickReplyInfo
...
export namespace Notify {
  export const getTemplaters = (user: UserData) => {
    const templateBody = (body: string) => {
      const templateFunc = new Function("user", "return `" + body + "`;");
    ...
    const templateRoute = (body: string) => {
      const templateFunc = new Function("route", "return `" + body + "`;");
      return String(templateFunc(body));
    };
    ...
    return { templateBody, templateButton, templateRoute };
  };
}
